...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = x.reshape(N,D).mm(w) + b #reshape和transpose的区别素？
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        #分成两部分，Linear_ReLU + Linear 
        #首先前向传播计算scores，先算LR部分，LR单元的forwar返回的是LR单元的输出和LR单元中L的输入和R的输入
        Linear_ReLU_out, param4back = Linear_ReLU.forward(X,self.params['W1'],self.params['b1'])
        #再算L部分：返回值是L单元的输出和L单元的输入（包括x,w,b）
        scores,_ = Linear.forward(Linear_ReLU_out,self.params['W2'],self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dout /= N
        #用L单元的backward计算W2、b2的梯度
        #backward的输入是上游梯度,这层单元的输入（包括xwb）
        dlr_in,grads['W2'],grads['b2']=Linear.backward(dout,(Linear_ReLU_out,self.params['W2'],self.params['b2']))
        #再用LR单元的backward计算W1，b1的梯度
        #其输入是上游梯度、L的输入、R的输入
        grads['W2'] += 2 * self.reg * self.params['W2']
        dx,grads['W1'],grads['b1']=Linear_ReLU.backward(dlr_in,param4back)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1,L-1):
            keyW = 'W' + str( i + 1 )
            keyb = 'b' + str( i + 1 )
            self.params[keyW] = torch.normal(0.0,weight_scale,size=(hidden_dims[i-1],hidden_dims[i]),dtype=dtype,device=device)
            self.params[keyb] = torch.zeros(hidden_dims[i],dtype=dtype,device=device)
        keyW = 'W' + str(L)
        keyb = 'b' + str(L)
        self.params[keyW] = torch.normal(0.0,weight_scale,size=(hidden_dims[-1],num_classes),dtype=dtype,device=device)
        self.params[keyb] = torch.zeros(num_classes,dtype=dtype,device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        param4back = []
        masks = []
        scores,param = Linear_ReLU.forward(X,self.params['W'+str(1)],self.params['b'+str(1)])
        param4back.append(param)
        if self.use_dropout:
            scores,temp = Dropout.forward(scores,self.dropout_param)
            masks.append(temp[1])
        for i in range(1,L-1):
          scores,param = Linear_ReLU.forward(scores,self.params['W'+str(i+1)],self.params['b'+str(i+1)])
          param4back.append(param)
          if self.use_dropout:
            scores,temp = Dropout.forward(scores,self.dropout_param)
            masks.append(temp[1])
        scores,param = Linear.forward(scores,self.params['W'+str(L)],self.params['b'+str(L)])
        param4back.append(param)
        #dropout
        ...
        for i in range(L):
            key = 'W' + str(i+1)
            loss +=  self.reg * torch.sum(self.params[key] * self.params[key]) 
        #{linear - relu - [dropout]} x (L - 1) - linear - softmax
        #先dloss/dscore，抄一下之前写的
        dscores = torch.zeros_like(scores)
        dscores[torch.arange(N),y] = -1
        dscores += torch.exp(scores) / torch.sum(torch.exp(scores),dim=1).unsqueeze(1)
        dscores /= N
        
        #再线形层
        dx,grads['W'+str(L)],grads['b'+str(L)] = Linear.backward(dscores,param4back[-1])
        #再L-1层LR层
        for i in range(L-1):
          if self.use_dropout:
            dx =  Dropout.backward(dx,(self.dropout_param,masks[L-2-i]))
          dx,grads['W'+str(L-1-i)],grads['b'+str(L-1-i)] = Linear_ReLU.backward(dx,param4back[L-2-i])
        for i in range(L):
            key = 'W' + str(i+1)
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict,
          ...
          #  print_every=1000,
           device=device)
    #solver.train() 后面有train，不是这里train
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def get_three_layer_network_params():
    ...
    learning_rate = 0.25  # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                             END OF YOUR CODE                 #
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    learning_rate = 1e-1#2e-3  # Experiment with this!
    weight_scale = 1e-1+1e-2#1e-5   # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                       END OF YOUR CODE                       #
...
def rmsprop(w, dw, config=None):
    ...
    config.setdefault('epsilon', 1e-8)#一般设为1e-4到1e-8之间
    ...
    next_w = w - config['learning_rate'] * dw / (torch.sqrt(cache + config['epsolon']))
    config['cache'] = cache
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    vc = v / (1- (beta2**config['t']))

    next_w = w - config['learning_rate'] * mt / (torch.sqrt(vc) + eps)
    config['m'] = m
    config['v'] = v
    #########################################################################
    #                              END OF YOUR CODE                         #
