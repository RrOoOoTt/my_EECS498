...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    #scores = torch.exp(scores)为啥得分不概率分布化啊——算损失的时候才概率分布化
    #scores /= torch.sum(scores,dim=1).unsqueeze(1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    loss = -torch.sum(torch.log(torch.exp(correct_class_score)/torch.sum(torch.exp(scores),dim=1)))
    loss /= N
    ...
    # Replace "pass" statement with your code
    #输出层梯度
    dloss_output = torch.zeros_like(scores)
    dloss_output[torch.arange(N),y] -= 1
    dloss_output += torch.exp(scores)/torch.sum(torch.exp(scores),dim=1).unsqueeze(1)
    dloss_output /= N
    
    #偏置b2的梯度
    grads['b2'] = torch.sum(dloss_output,dim=0)
    #权重W2的梯度
    grads['W2'] = 2 * reg * W2 + h1.t().mm(dloss_output)#h1.t() * dloss_output
    #注意ReLU的反向传播
    dh = dloss_output.mm(W2.t())#W2.t() * dloss_output
    dh[h1 <= 0] = 0
    #偏置b1的梯度
    grads['b1'] = torch.sum(dh,dim = 0)
    #权重W1的梯度
    grads['W1'] = 2 * reg * W1 + X.t().mm(dh)#X.t() * dReLU
...
def nn_predict(
    ...
    y_pred = X.mm(W1) + b1
    y_pred[y_pred <= 0] = 0
    y_pred = (y_pred.mm(W2) + b2).argmax(dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def find_best_net(
    ...
    for lr in lrs:
        for lrd in lrds:
            for hs in hss:
                for reg in regs:
                    net = TwoLayerNet(X_train.shape[1],hs,y_train.shape[0],dtype=X_train.dtype,device=X_train.device)
                    stats = net.train(X_train,y_train,X_val,y_val,num_iters=3000,batch_size=1000,learning_rate=lr,learning_rate_decay=lrd,reg=reg,verbose=False)
