...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        #零填充
        #During padding, 'pad' zeros should be placed symmetrically (i.e equally
        #on both sides) along the height and width axes of the input.
          ...
          for f in range(F):
            for h in range(H_out):
              for w_ in range(W_out):
                out[n,f,h,w_] = torch.sum(w[f] * x[n,:,h*stride:h*stride+HH,w_*stride:w_*stride+WW]) + b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
    ...
    def backward(dout, cache):
        ...
        #dx_padded = torch.nn.functional.pad(x, (pad, pad, pad, pad))
          ...
          for f in range(F):
            for h in range(H_out):
              for wi in range(W_out):
                db[f] += dout[n,f,h,wi]
                dw[f] += x[n,:,h*stride:h*stride+HH,wi*stride:wi*stride+WW] * dout[n,f,h,wi] 
                dx[n,:,h*stride:h*stride+HH,wi*stride:wi*stride+WW] += w[f] * dout[n,f,h,wi]
        if pad > 0:
          ...
          for c in range(C):
            for h in range(H_out):
              for w in range(W_out):
                out[n,c,h,w] = torch.max(x[n,c,h*stride:h*stride+pool_height,w*stride:w*stride+pool_width])#返回值是最大值
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        #池化层 2x2 max pool
        H_out = 1 + (H_out - 2) // 2#2x2的池化层的步幅是2吗
        W_out = 1 + (W_out - 2) // 2
        #隐藏层 linear - relu
        self.params['W2'] = torch.normal(0.0,weight_scale,(num_filters*H_out*W_out,hidden_dim),dtype=dtype,device=device)#虽然不知道为什么但是要展开
    ...
    def loss(self, X, y=None):
        ...
        #conv - relu - 2x2 max pool - linear - relu - linear - softmax
        param4back = []
        scores,param = Conv_ReLU_Pool.forward(X, W1, b1, conv_param,pool_param)
        param4back.append(param)
        scores,param = Linear_ReLU.forward(scores, W2, b2)
        param4back.append(param)
        scores,param = Linear.forward(scores,W3,b3)
        param4back.append(param)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        N,C,H,W = X.shape
        loss = -torch.sum(torch.log(torch.exp(scores)[torch.arange(N),y] / torch.sum(torch.exp(scores),dim=1)))
        loss /= N
        loss += self.reg*( torch.sum(W1*W1) + torch.sum(W2*W2) + torch.sum(W3*W3))
        #可能改天用微分法推导一下
        dout = torch.zeros_like(scores)
        dout[torch.arange(N),y] = -1
        dout += torch.exp(scores) / torch.sum(torch.exp(scores),dim=1).unsqueeze(1)
        dout /= N
        #Linear
        dx, grads['W3'], grads['b3'] = Linear.backward(dout,param4back[2])
        #Linear_ReLU
        dx, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx,param4back[1])
        #Conv_ReLU_Pool          
        dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dx,param4back[0])
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        # Replace "pass" statement with your code
        #passkernel size 3 and
        #padding 1
        # max pooling layers with 2x2 receptive fields and a stride of 2
        #{conv - [batchnorm?] - relu - [pool?]} x (L - 1) - linear
        L = self.num_layers
        num_channel,H_out,W_out = input_dims
        pre_filter = num_channel
        filter_size = 3
        stride = 1
        pad = 1
        pool_height = 2
        ...
        for i in range(L-1):
          ...
          H_out = 1 + (H_out - HH + 2 * pad) // stride#还没写loss实现，但改成向零取整就对了..公式记错了
          W_out = 1 + (W_out - WW + 2 * pad) // stride
          #H_out = int(1+(H_out - HH + 2 * pad) / stride)
          #W_out = int(1+(W_out - WW + 2 * pad) / stride)
          num_filter = num_filters[i]
          
          keyb = 'b' + str(i+1)
          self.params[keyb] = torch.zeros(num_filter,dtype=dtype,device=device)
          if self.batchnorm:
              self.params['gamma'+str(i+1)] = torch.ones(num_filter,dtype=dtype,device=device)
              self.params['beta'+str(i+1)] = torch.zeros(num_filter,dtype=dtype,device=device)
          if i in max_pools:
              # H_out = (H_out - 2) // 2
              # W_out = (W_out - 2) // 2
              H_out = 1 + (H_out - pool_height) // pool_stride
              W_out = 1 + (W_out - pool_width) // pool_stride
              #H_out = int(1+(H_out - pool_height) / pool_stride)
              #W_out = int(1+(W_out - pool_width) / pool_stride)
          if weight_scale == 'kaiming':
              torch.nn.init.kaiming_normal_(self.params[keyW], mode='fan_in', nonlinearity='relu')
          else:
              self.params[keyW] = torch.normal(0.0,weight_scale,(num_filter,pre_filter,filter_size,filter_size),dtype=dtype,device=device)#why prefilter
        ...
        if weight_scale == 'kaiming':
            ...
            self.params[key] = torch.normal(0.0,weight_scale,(num_filters[-1]*H_out * W_out,num_classes),dtype=dtype,device=device)#shape
        key = 'b' + str(L)
        self.params[key] = torch.zeros(num_classes,dtype=dtype,device=device)
        #有必要总结一下
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(L-1):
          ...
          if i in self.max_pools:
            if self.batchnorm:
                scores,param = Conv_BatchNorm_ReLU_Pool.forward(scores, self.params[keyW], self.params[keyb],
                                                                self.params['gamma{}'.format(i+1)],self.params['beta{}'.format(i+1)],
                                                                conv_param,self.bn_params[i],pool_param)
                param4back.append(param)
            else:
                scores,param = Conv_ReLU_Pool.forward(scores, self.params[keyW], self.params[keyb], conv_param,pool_param)
                param4back.append(param)
          else:
              if self.batchnorm:
                  scores,param = Conv_BatchNorm_ReLU.forward(scores, self.params[keyW], self.params[keyb], 
                                                             self.params['gamma{}'.format(i+1)],self.params['beta{}'.format(i+1)],
                                                             conv_param,self.bn_params[i])
                  param4back.append(param)
              else:
                  scores,param = Conv_ReLU.forward(scores, self.params[keyW], self.params[keyb], conv_param)
                  param4back.append(param)
        
        keyW = 'W' + str(L)
        keyb = 'b' + str(L)
        scores,param = Linear.forward(scores,self.params[keyW],self.params[keyb])
        param4back.append(param)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        #似乎这里有个隐式类型转换，loss从int变成tensor了
        #先不考虑批归一化
        #{conv - [batchnorm?] - relu - [pool?]} x (L - 1) - linear
        for i in range(L):
        ...
        #接下来算grads
        #Linear
        dout, grads['W{}'.format(L)], grads['b{}'.format(L)] = Linear.backward(dout,param4back[-1])
        for i in range(L-1,0,-1):
            j = i - 1#这就是index不对齐的下场
            if j in self.max_pools:
                dout, grads['W{}'.format(i)], grads['b{}'.format(i)] = Conv_ReLU_Pool.backward(dout,param4back[i-1])
            else:
                dout, grads['W{}'.format(i)], grads['b{}'.format(i)] = Conv_ReLU.backward(dout,param4back[i-1])
        for i in range(L):
