...
def svm_loss_naive(
    ...
    num_classes = W.shape[1] #分类数
    num_train = X.shape[0] #训练样本数
    loss = 0.0
    for i in range(num_train):
        scores = W.t().mv(X[i]) #计算每个类别的得分
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                #part1:这里只是计算梯度，还没有用梯度来更新W，不要与梯度下降中的负梯度方向混淆
                dW[:, y[i]] -= X[i].t()
                dW[:, j] += X[i].t()
                #######################################################################
                #                       END OF YOUR CODE                              #
...
def svm_loss_vectorized(
    ...
    #先wx内积再max再求和
    num_train = X.shape[0]
    scores = torch.mm(X,W)#X.mm(W) X.matmul(W) X.dot(W)
    correct_class_score = scores[torch.arange(num_train), y].view(-1,1)#reshape(-1,1)
    margin = scores - correct_class_score + 1
    #忘记把正确分类的得分置为0了
    margin[torch.arange(num_train), y] = 0#?
    #loss = torch.max(margin,dim=1)[0].sum()这一行写错了
    mask = (margin > 0)
    ...
    #mask2 = (margin < 0)
    margin[margin < 0] = 0
    margin[torch.arange(num_train), y] = -torch.sum(margin, dim=1)
    # indicator = torch.zeros_like(scores)#且点乘
    # # indicator = torch.zeros(margin.shape)
    # indicator[margin > 0] = 1
    # indicator[torch.arange(num_train), y] = -torch.sum(indicator, dim=1)
    # dW = X.t().mm(indicator)
    dW = X.t().mm(margin)
    dW /= num_train
    dW += 2 * reg * W
...
def svm_get_search_params():
    ...
    for i in range(5):
        ...
        regularization_strengths.append(1*10**(-i))
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    num_iters = 100 
    # Replace "pass" statement with your code
    _=cls.train(data_dict['X_train'],data_dict['y_train'], lr, reg, num_iters=100,
...
def softmax_loss_naive(
    ...
    score = X.mm(W)
    for i in range(num_train):
        temp = torch.sum(torch.exp(score[i]))
        Li = -score[i][y[i]] + torch.log(temp)
        loss += Li
        dW[:,y[i]] -= X[i].t()
        for j in range(num_classes):
            dW[:,j] += torch.exp(score[i][j]) * X[i].t() / temp #用X[i].T有warning
    loss /= num_train
    dW /= num_train
    ##添加正则
    loss += reg * torch.sum(W * W)
...
def softmax_loss_vectorized(
    ...
    num_classes = W.shape[1]
    score = X.mm(W)
    correct_class_score = score[torch.arange(num_train),y]
    loss -= torch.sum(torch.log(torch.exp(correct_class_score)/torch.sum(torch.exp(score),dim=1)))
    loss /= num_train
    loss += reg * torch.sum(W*W)
    mask = torch.zeros(num_classes,num_train,dtype=torch.float64,device = 'cuda')
    mask[y,torch.arange(num_train)] = 1

    escore = torch.exp(score)
    escore /= torch.sum(escore,dim=1).unsqueeze(1)
    dW = torch.mm(escore.t(),X).t()
...
def softmax_get_search_params():
    ...
    - regularization_strengths: regularization strengths candidates
                                e.g. [1e0, 1e1, ...]
    """
    learning_rates = [1e-3,1e-2,1e-4,1e-5,1e-1]
    regularization_strengths = [1e-2,1e-3,1e-1]
